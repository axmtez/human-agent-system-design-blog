import * as THREE from 'three';
import { OrbitControls } from 'three/addons/controls/OrbitControls.js';
import Stats from 'stats.js';
import { initTestScene, buildComposedScene, updateAmbientAnimations } from '../lib/diorama';

function run(): void {
  const container = document.getElementById('diorama-container');
  if (!container) return;

  const { scene, camera, renderer, updateFrustum } = initTestScene(container);
  buildComposedScene(scene);

  const clock = new THREE.Clock();

  const controls = new OrbitControls(camera, renderer.domElement);
  controls.enableDamping = true;
  controls.dampingFactor = 0.05;

  const stats = new Stats();
  stats.showPanel(0);
  stats.dom.style.position = 'absolute';
  stats.dom.style.left = '0';
  stats.dom.style.top = '0';
  container.appendChild(stats.dom);

  const overlay = document.getElementById('diorama-scene-info');
  function updateOverlay(): void {
    if (!overlay) return;
    overlay.textContent = 'Ambient: beacon blink, nav pulse, holding orbit, tower glow';
  }

  let animationId: number;
  function animate(): void {
    animationId = requestAnimationFrame(animate);
    const elapsed = clock.getElapsedTime();
    updateAmbientAnimations(scene, elapsed);
    controls.update();
    renderer.render(scene, camera);
    stats.update();
    updateOverlay();
  }

  const ro = new ResizeObserver(() => updateFrustum());
  ro.observe(container);
  window.addEventListener('resize', () => updateFrustum());

  animate();
  updateOverlay();
}

if (typeof document !== 'undefined') {
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', run);
  } else {
    run();
  }
}
